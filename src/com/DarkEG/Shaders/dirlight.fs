...
vec4 calcLight(vec3 lightColor, vec3 lightPosi, vec3 LightDir, vec3 worldPos, vec3 Normal){
	vec4 AmbientColor = vec4(lightColor, 1.0) * 0.3;
	...
	if(DiffuseFactor > 0.0){
		DiffuseColor = vec4(lightColor, 1.0) * 0.3 * DiffuseFactor;
...
vec4 calcPointLight(vec3 worldPos, vec3 Normal){
	...
	return calcLight(lightCol, lightPos, LightDir, worldPos, Normal);
}
void main(){
